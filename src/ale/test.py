...
def ball_position(obs):
    ball_pixels = np.argwhere(obs[:, 16:79] > 0.8)

    ball_y = float(np.mean(ball_pixels[:, 0])) if len(ball_pixels) > 0 else None

    paddle_pixels = np.argwhere(obs[:, 70:79] > 0.4)
    ...
    try:
        for episode in range(start, config.EPISODES):
            ...
            while True:
                ...
                ball_y, paddle_y = ball_position(next_state[-1])
                ...
                if ball_y is not None and paddle_y is not None:
                    ...
                    track = config.DISTANCE_REWARD * (1-distance / 84.0)
                    reward += track
