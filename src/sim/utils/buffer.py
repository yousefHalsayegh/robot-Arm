...
def get_native_rgb_frame(base_env) -> np.ndarray:
    rgb = base_env.scene["side"].data.output["rgb"][0].cpu().numpy()  # [720, 1280, 3] or similar, env index 0
        ...
        lerobot_dataset = LeRobotDataset.create(
            ...
            features={
                ...
                "action": {"dtype": "float32", "shape": (6,),
                    "names": ["shoulder_pan.pos", "shoulder_lift.pos", "elbow_flex.pos",
                              "wrist_flex.pos", "wrist_roll.pos", "gripper.pos"]},
                "observation.images.front": {"dtype": "video", "shape": (720, 1280, 3),
                    ...
                    if not warmup:  # only the final hop's windows get pushed — every window, not just the last
                        ...
                        if lerobot_dataset is not None:
                            ...
                            lerobot_dataset.add_frame({
                                ...
                                "observation.images.front": get_native_rgb_frame(base_env),   # not get_newest_rgb_frame(cam_next)
...
def main(args_cli, simulation_app):
    ...
    generate_synthetic_transitions(
            ...
            device=device,
            simulation_app=simulation_app,
            export_lerobot= args_cli.export_lerobot,
            brain=None, 
            steps_counter=0
        )
